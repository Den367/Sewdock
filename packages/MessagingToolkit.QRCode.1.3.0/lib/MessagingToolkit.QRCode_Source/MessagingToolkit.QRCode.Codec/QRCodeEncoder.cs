...
namespace MessagingToolkit.QRCode.Codec
{
    ...
    public class QRCodeEncoder
    {
        ...
        internal int qrcodeVersion = 0;

        public QRCodeEncoder()
        {
            this.CharacterSet = DEFAULT_ENCODING;
        }
        ...
        public virtual bool[][] CalQrcode(byte[] qrcodeData)
        {
            ...
            try
            {
                embeddedFile = GetEmbeddedFile(DATA_PATH + ".qrv" + Convert.ToString(this.qrcodeVersion) + "_" + Convert.ToString(num7) + ".dat");
            ...
            sbyte num12 = 1;
            for (byte i = 1; i < 0x80; i = (byte) (i + 1))
            {
                if (numArray13[i] == 0)
                {
                    num12 = (sbyte) i;
            ...
            sbyte num25 = (sbyte) ((num7 << 3) | ((byte) num23));
        ...
        {
            return new Bitmap(GetEmbeddedFile(fileName));
        }

        internal static XmlDocument GetEmbeddedXml(string fileName)
        {
            XmlTextReader reader = new XmlTextReader(GetEmbeddedFile(fileName));
