    ...
    public abstract class SiteControllerBase : Controller
    {
        ...
        protected override void Execute(RequestContext requestContext)
        {
            try
            {
                ...
                this.MasterViewModel = new MasterViewModel(this.SiteData, this.SiteData.Settings.Keywords, DateTimeOffset.UtcNow.AdjustFromUtc());
        ...
        private static void CheckFirstTimeInitialization()
        {
            using (var repositoryRead = MayandoRepositoryFactory.CreateRepository())
            {
                ...
                if (settings.Count == 0)
                {
                    using (var repositoryWrite = MayandoRepositoryFactory.CreateRepository(true))
                    {
                        ...
                        var photosMenu = new Menu
                        {
                            ...
                            Title = "Photos",
                            Url = "~/photos",
                            ToolTip = "Go to my latest photos",
                ...
                {
                    // There are settings already, upgrade from a previous version if needed.
                    var settingValues = repositoryRead.GetSettingValues(SettingsScope.Application);
                    var applicationSettings = new ApplicationSettings(settingValues);
                    if (string.IsNullOrEmpty(applicationSettings.LastRunVersion) || !string.Equals(applicationSettings.LastRunVersion, currentVersion))
                    {
                        // The last run version was different from the current version.
                        // Ensure new settings are available.
                        using (var repositoryWrite = MayandoRepositoryFactory.CreateRepository(true))
                        {
                            var lastVersion = (string.IsNullOrEmpty(applicationSettings.LastRunVersion) ? string.Empty : " from v" + applicationSettings.LastRunVersion);
                            logDetails.AppendFormat(CultureInfo.CurrentCulture, "Upgrading{0} to v{1}", lastVersion, currentVersion);
                            repositoryWrite.EnsureSettings(SettingsScope.Application, ApplicationSettings.GetSettingDefinitions());
                            repositoryWrite.CommitChanges();
                        }
                        shouldWriteVersion = true;
                ...
                if (shouldWriteVersion)
                {
                    ...
                    // Note that this needs to be done in a new transaction since the settings from a previous
                    // call have not yet been committed so the LastRunVersion might not be present otherwise.
                    using (var repositoryWrite = MayandoRepositoryFactory.CreateRepository(true))
                    {
                        var settingValues = new Dictionary<string, string>();
                        settingValues.Add(ApplicationSettings.SettingNameLastRunVersion, currentVersion);
                        repositoryWrite.SaveSettingValues(SettingsScope.Application, settingValues);
                        repositoryWrite.CommitChanges();
                    }
                }
        ...
        /// <param name="initializer">The initializer to run for the new model instance.</param>
        /// <returns>The Create view for a new instance of the specified model type.</returns>
        protected ActionResult ViewForCreate<TModel>(Action<MayandoRepository, TModel> initializer) where TModel : class, new()
        {
            using (var repository = GetRepository())
            {
                TModel model = new TModel();
        ...
        protected ActionResult ViewForDelete<TModel>(string itemName, Func<TModel, string> itemDescriptionSelector, Func<MayandoRepository, TModel> itemSelector)
        {
            using (var repository = GetRepository())
            {
        ...
        protected ActionResult ViewFor<TModel>(ViewName viewName, Func<MayandoRepository, TModel> itemSelector)
        {
            using (var repository = GetRepository())
            {
                TModel item = itemSelector(repository);
        ...
        #region PerformSave & PerformDelete

        /// <summary>
        /// Saves an entity to the repository.
        /// </summary>
        /// <typeparam name="TModel">The type of the model.</typeparam>
        /// <param name="viewName">The name of the view to show if the save did NOT succeeded.</param>
        /// <param name="entity">The entity to save.</param>
        /// <param name="save">Performs the actual save operation to the repository.</param>
        /// <returns>The specified view for the entity if the save did NOT succeed, or a redirection to the Index view if the save did succeed.</returns>
        protected ActionResult PerformSave<TModel>(ViewName viewName, TModel entity, Action<MayandoRepository> save)
        {
            return PerformSave<TModel>(viewName, entity, save, null);
        }
        ...
        /// <param name="entity">The entity to save.</param>
        /// <param name="save">Performs the actual save operation to the repository.</param>
        /// <param name="successResult">The action result to return if the save did succeed.</param>
        /// <returns>The specified view for the entity if the save did NOT succeed, or the specified result if the save did succeed.</returns>
        protected ActionResult PerformSave<TModel>(ViewName viewName, TModel entity, Action<MayandoRepository> save, ActionResult successResult)
        {
            ...
            using (var repository = GetRepository(true))
            {
                ...
                SetPageFlash("The item was saved.");
        ...
        /// <summary>
        /// Performs a delete of an entity.
        /// </summary>
        /// <param name="delete">Performs the actual delete operation from the repository.</param>
        /// <returns>A redirection to the Index view.</returns>
        protected ActionResult PerformDelete(Action<MayandoRepository> delete)
        {
            return PerformDelete(delete, ActionName.Index);
        }
        ...
        /// </summary>
        /// <param name="delete">Performs the actual delete operation from the repository.</param>
        /// <param name="redirectAction">The action to redirect to after the delete was performed.</param>
        /// <returns>A redirection to the specified action.</returns>
        protected ActionResult PerformDelete(Action<MayandoRepository> delete, ActionName redirectAction)
        {
            using (var repository = GetRepository(true))
            {
                delete(repository);
                repository.CommitChanges();
                SetPageFlash("The item was deleted.");
        ...
        protected static MayandoRepository GetRepository()
        {
            return MayandoRepositoryFactory.CreateRepository(false);
        }

        /// <summary>
        /// Gets the repository.
        /// </summary>
        /// <param name="writable">Determines if a transaction should be used around the entire lifetime of the repository.</param>
        /// <returns>The repository.</returns>
        protected static MayandoRepository GetRepository(bool writable)
        {
            return MayandoRepositoryFactory.CreateRepository(writable);
        }
