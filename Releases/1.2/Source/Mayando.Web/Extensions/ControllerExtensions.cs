...
namespace Mayando.Web.Extensions
{
    public static class ControllerExtensions
    {
        ...
        {
            return string.Format(CultureInfo.InvariantCulture, "Comment from {0} on {1}", comment.AuthorName, comment.Photo.DisplayTitle);
        }
        ...
        public static string GetCommentNotificationText(this SiteControllerBase controller, Comment comment, bool includeAdminLinks)
        {
            ...
            text.Append("<div>");
            text.AppendFormat(CultureInfo.InvariantCulture, "<div><img src=\"{0}\" /></div>", comment.Photo.UrlSmallestAvailable);
        ...
        public static Comment AddCommentToPhoto(this SiteControllerBase controller, int photoId, Comment comment, bool rememberMe)
        {
            ...
            else
            {
                ...
                using (var repository = MayandoRepositoryFactory.CreateRepository())
                {
                    ...
                    {
                        Logger.Log(LogLevel.Warning, string.Format(CultureInfo.CurrentCulture, "The comment from {0} on '{1}' was not checked for spam by the provider; it will be posted anyway so please validate the comment manually.", savedComment.AuthorName, savedComment.Photo.DisplayTitle));
                    }
                    if (result.Classification == Classification.Unsure)
                    {
                        Logger.Log(LogLevel.Warning, string.Format(CultureInfo.CurrentCulture, "The comment from {0} on '{1}' was checked for spam but the provider wasn't sure if it is spam or not; it will be posted anyway so please validate the comment manually.", savedComment.AuthorName, savedComment.Photo.DisplayTitle));
                    }

                    Logger.Log(LogLevel.Information, string.Format(CultureInfo.CurrentCulture, "New comment from {0} on '{1}'", savedComment.AuthorName, savedComment.Photo.DisplayTitle), savedComment.Text);
        ...
        #region Photo Size Helper Methods

        /// <summary>
        /// Gets a photo details view model.
        /// </summary>
        /// <param name="controller">The controller.</param>
        /// <param name="photo">The photo.</param>
        /// <param name="hidePhotoText">Determines if the text of the photo should be hidden in the view.</param>
        /// <param name="hidePhotoComments">Determines if the comments of the photo should be hidden in the view.</param>
        /// <returns>A model to be used to show details about a photo.</returns>
        public static PhotoDetailsViewModel GetPhotoDetailsViewModel(this SiteControllerBase controller, Photo photo, bool hidePhotoText, bool hidePhotoComments)
        {
            ...
            if (!string.IsNullOrEmpty(explicitSize))
            {
                ...
                controller.Response.Cookies.Set(new HttpCookie("PhotoSize", explicitSize) { Expires = DateTime.Now.AddDays(14) });
            }
            var preferredSizePhotoUrl = photo.UrlNormal;
            var preferredSize = PhotoSize.Normal;
            if (string.Equals(SiteControllerBase.ParameterPhotoSizeLarge, preferredPhotoSize, StringComparison.OrdinalIgnoreCase) && !string.IsNullOrEmpty(photo.UrlLarge))
            {
                preferredSizePhotoUrl = photo.UrlLarge;
                preferredSize = PhotoSize.Large;
            }
            else if (string.Equals(SiteControllerBase.ParameterPhotoSizeSmall, preferredPhotoSize, StringComparison.OrdinalIgnoreCase) && !string.IsNullOrEmpty(photo.UrlSmall))
            {
                preferredSizePhotoUrl = photo.UrlSmall;
                preferredSize = PhotoSize.Small;
            }
            return new PhotoDetailsViewModel(photo, preferredSizePhotoUrl, preferredSize, hidePhotoText, hidePhotoComments, controller.SiteData.Settings.LinkToExternalPhotoUrl);
        }
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "controller")]
        public static IPagedList<Photo> GetPhotosForGallery(this SiteControllerBase controller, MayandoRepository repository, Gallery gallery, int count, int page)
        {
            var tags = gallery.Tags.Select(t => t.Name).ToList();
            var photos = repository.GetPhotosWithCommentsTagged(PhotoDateType.Published, tags, count, page);
            return photos;
        }
        ...
        public static void StorePhotoListInViewData(this ControllerBase controller, int? selectedPhotoId, bool allowNull)
        {
            using (var repository = MayandoRepositoryFactory.CreateRepository())
            {
                var photos = repository.GetPhotos(PhotoDateType.Published);
